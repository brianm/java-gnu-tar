...
public class TarFileUtil
{
     ...
     */
    public static int getFileNameBytes(String newName, byte[] outbuf, boolean isGNUTar) throws InvalidHeaderException
    {
        if (!isGNUTar && newName.length() > 100) {
     ...
     * @return The long value of the octal bytes.
     */
    public static int getLongOctalBytesMulti(long value, byte[] buf, int offset, int length)
    {
    ...
    public static long parseOctal(byte[] header, int offset, int length)
    {
        ...
        for (int i = offset; i < end; ++i) {
            ...
            if (header[i] == TarConstants.SPACER_BYTE
                || header[i] == TarConstants.ZERO_BYTE)
            { // == '0'
                if (stillPadding) {
                    continue;
                }
